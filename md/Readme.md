# Anson Pinhero

> Software Engineer &nbsp;|&nbsp; Photographer

Architecting robust, scalable, and secure systems for high-compliance domains.

---

## About Me

Hi, I'm Anson. I'm a Full Stack Software Engineer with **nearly 5 years of experience** specializing in solution design and end-to-end application development.

My expertise lies in architecting and building scalable systems, particularly for complex, high-compliance domains like healthcare. I have deep experience in **modern backend technologies** (ASP.NET Core and Spring Boot), **cloud services** (Microsoft Azure and AWS), and a strong focus on **Generative AI**.

I hold a **Bachelor of Technology in Computer Science & Engineering** from APJ Abdul Kalam Technological University. I'm passionate about building high-impact products that make the world a better place.

Beyond the code, I'm an avid photographer. I see both engineering and photography as forms of storytelling — one builds a narrative with code, the other with light. Both require finding order in complexity.

---

## Skills & Technologies

**Backend:** ASP.NET Core (C#), Spring Boot (Java), REST APIs, Microservices  
**Cloud:** Microsoft Azure, Amazon Web Services (AWS)  
**AI / ML:** Generative AI, LLMs, Retrieval-Augmented Generation (RAG)  
**Domains:** Healthcare (HIPAA compliance), High-compliance systems  

---

## Research

I have published work in the areas of malware detection and Android security. See my [Research page](/md/research.html) or [Google Scholar profile](https://scholar.google.com/citations?user=v8l_TBMAAAAJ&hl=en) for full details.
